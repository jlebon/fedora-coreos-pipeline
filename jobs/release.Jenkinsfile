            ...
            stage("Push Containers") {
                parallel push_containers.collectEntries{configname, val -> [configname, {
                    withCredentials([file(variable: 'REGISTRY_SECRET',
                        ...
                        if (pipecfg.hotfix) {
                            tag_suffix += "-${pipecfg.hotfix.name}"
                        }
...
// main try finishes here
    ...
    if (currentBuild.result != 'SUCCESS') {
        trySlackSend(color: 'danger', message: ":fcos: :bullettrain_front: :trashfire: release <${env.BUILD_URL}|#${env.BUILD_NUMBER}> [${params.STREAM}][${params.ARCHES}] (${params.VERSION})")
    }
